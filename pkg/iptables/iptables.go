// Copyright 2022 Syntropy.Net
// This code is based on https://github.com/coreos/go-iptables
// Motivation for fork: a need to explicitely require -legacy or -nft tables.
...
func IptVariant(v Variant) option {
	return func(ipt *IPTables) {
		ipt.variant = v
	}
}
...
func New(opts ...option) (*IPTables, error) {
	...
	path, err := exec.LookPath(getIptablesCommand(ipt.proto, ipt.variant))
	if err != nil {
		return nil, err
	}
	ipt.path = path
...
func getIptablesCommand(proto Protocol, variant Variant) string {
	var cmd string
